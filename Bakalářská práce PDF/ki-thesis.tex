...
Konceptuální návrh DWH se opírá o dvě hlavní, avšak protichůdné, metodiky, které se liší přístupem k datové integritě, flexibilitě a rychlosti dodání výsledků:
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Schema:} DWH je modelováno ve vysoce \textbf{normalizované} formě (typicky 3. normální forma, 3NF), což zajišťuje nízkou datovou redundanci, maximální integritu a vysokou flexibilitu pro budoucí požadavky.
        \item \textbf{Tok dat a tržiště:} Data jsou nejprve ETL procesem načtena do normalizovaného EDW. Teprve z něj jsou následně vytvářena menší, tématicky specializovaná, denormalizovaná \textbf{datová tržiště} (\textit{Data Marts}) pro koncové reportovací nástroje.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Tok dat a tržiště:} Data jsou transformována a ukládána přímo do dimenzionálních modelů. V tomto modelu \textbf{je datové tržiště ekvivalentní} dimenzionálně modelované sadě tabulek Fakta/Dimenze. Podnikový datový sklad je pak \textbf{logickou unií} těchto datových tržišť, propojených pomocí \textbf{konformních dimenzí} (\textit{Conformed Dimensions}).
    \end{itemize}
\end{enumerate}
V rámci tohoto projektu byla s ohledem na optimalizaci výkonu, zjednodušení dotazování pro sémantickou vrstvu (Cube.js) a agilní vývoj zvolena \textbf{Kimballova metodika}.
...
\begin{itemize}
    ...
    Tato vrstva nahrazuje tradiční prezentační vrstvu a slouží jako \textbf{jednotná definice obchodní logiky} a metrik nad daty z DWH. Je reprezentována nástrojem \textbf{Cube.js} (viz \texttt{docker-compose2.yml}), který zajišťuje předagregaci, cachování a standardizované výpočty. Koncovým BI nástrojům (jako je Apache Superset) pak zpřístupňuje data prostřednictvím \textbf{SQL API} nebo \textbf{GraphQL}, čímž se zjednodušuje dotazování a zvyšuje výkon.
\end{itemize}
