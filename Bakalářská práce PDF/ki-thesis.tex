...
\begin{itemize}
    ...
    \item \textbf{Open-source software} má veřejně dostupný zdrojový kód, který lze volně používat, upravovat a distribuovat. Hlavní výhodou jsou nízké nebo žádné licenční náklady, vysoká míra flexibility a silná komunita vývojářů. Nevýhodou může být absence centrální podpory a vyšší nároky na technické znalosti při implementaci.
...
\textit{(Zde bude doplněn obrázek znázorňující ekosystém Microsoft Data Platform)}
...
Silnou stránkou tohoto ekosystému je jeho vzájemná provázanost – od správy dat přes analytické modely až po prezentaci. Díky hluboké integraci s prostředím \emph{Microsoft Azure} lze tento systém provozovat v hybridní nebo plně cloudové architektuře (např. \emph{Azure Synapse Analytics}, \emph{Fabric}), ale i čistě on-premise. Nevýhodou může být proprietární charakter a závislost na licenční politice Microsoftu.
...
\textit{(Zde bude doplněn obrázek znázorňující ekosystém Google Cloud)}
...
Ekosystém Google se vyznačuje vysokou úrovní automatizace, integrací s nástroji pro strojové učení (např. \emph{Vertex AI}) a dostupností v rámci pay-as-you-go modelu. Jedná se o čistě cloudové řešení, což může být limitující pro organizace preferující on-premise infrastrukturu.
...
\textit{(Zde bude doplněn obrázek znázorňující ekosystém AWS)}
...
\subsection{Open-source datové ekosystémy (open-source, on-premise/cloud)}

Na rozdíl od komerčních platforem, které nabízejí uzavřená a plně integrovaná řešení, jsou open-source datové ekosystémy založeny na principu modularity. Jednotlivé komponenty – databázové systémy, nástroje pro modelování dat, semantické vrstvy a vizualizační rozhraní – lze často volně kombinovat a integrovat podle potřeb konkrétního projektu. Tento přístup umožňuje vysokou míru flexibility a přizpůsobení architektury bez závislosti na jednom dodavateli. Lze je provozovat jak on-premise, tak v cloudu na virtuálních strojích.
...
\hline
\multicolumn{2}{|c|}{\cellcolor{gray!20}\textbf{Vrstva: Datový sklad}} \\
\hline
...
\pagebreak
\hline
\multicolumn{2}{|c|}{\cellcolor{gray!20}\textbf{Vrstva: Sémantická vrstva / OLAP}} \\
\hline
\cellcolor{osgreen}
    \textbf{Cube.js} \par
...
\hline
\hline
\multicolumn{2}{|c|}{\cellcolor{gray!20}\textbf{Vrstva: Vizualizace / BI}} \\
\hline
\cellcolor{osgreen}
...
Cílem této kapitoly je objektivní srovnání výkonu implementovaných datových skladů. Analýza je navržena tak, aby otestovala klíčové operace typické pro analytickou zátěž a odhalila výkonnostní rozdíly mezi jednotlivými databázovými platformami a jejich úložnými enginy.

\subsection{Metodika měření}

Pro zajištění konzistentních a srovnatelných výsledků byla stanovena jasná metodika, která se skládá z několika kroků. Měření probíhalo ve dvou hlavních fázích: první fáze se zaměřila na výkon tradičního řádkového úložiště a druhá na výkon moderního sloupcového úložiště.

\subsubsection{Standardizace prostředí a indexace}

Před samotným měřením bylo nutné zajistit, aby prostředí pro srovnávací testy byla co nejvíce konzistentní. Výchozí datové jezero obsahovalo pro všechny technologie stejný objem surových dat. Rozdíl v implementaci nastal v prvním ETL kroku (z datového jezera do staging vrstvy): zatímco pro většinu databází byl zpracováván pouze topik týkající se kamerového systému města Bílina, pro PostgreSQL byl použit dynamický ETL skript, který do staging vrstvy načetl data z více topiků. Pro samotné výkonnostní měření však bylo klíčové, že v navazujícím ETL procesu (ze stagingu do finálního datového skladu) byla pro všechny databáze vybrána pouze data z topiku kamerového systému Bílina. Tím bylo zajištěno, že finální dimenzionální model — tedy tabulky faktů a dimenzí, nad kterými probíhalo měření — obsahoval pro všechny testované varianty naprosto identický objem dat. Všechny implementace rovněž sdílely shodné datové schéma hvězdy.

Klíčovým krokem byla standardizace indexace napříč platformami. U všech databází byly v první fázi vytvořeny standardní B-stromové indexy na cizích klíčích faktové tabulky a na relevantních sloupcích v dimenzionálních tabulkách, jak je uvedeno níže. Cílem bylo poskytnout každé databázi stejné výchozí podmínky pro optimalizaci dotazů v řádkovém režimu.

\begin{verbatim}
-- Indexy na faktové tabulce
CREATE INDEX IX_FactCameraDetection_TimeKey ON FactCameraDetection (TimeKey);
CREATE INDEX IX_FactCameraDetection_VehicleClassKey ON FactCameraDetection (VehicleClassKey);
-- ... (a další klíče)

-- Příklad indexu na dimenzi
CREATE INDEX IX_DimCity_Name ON DimCity (CityName);
\end{verbatim}

\subsubsection{Sada testovacích dotazů}

Byla připravena sada pěti reprezentativních SQL dotazů, které simulují běžné analytické úlohy. Dotazy byly navrženy tak, aby otestovaly různé aspekty výkonu databáze, od jednoduchých agregací po komplexní spojení s filtrováním a použitím okenních funkcí. Syntaxe dotazů byla dle potřeby upravena pro konkrétní SQL dialekt testované databáze.

\begin{itemize}
    \item \textbf{Dotaz 1:} Jednoduchá agregace s jedním spojením (průměrná rychlost podle třídy vozidla).
    \item \textbf{Dotaz 2:} Agregace nad časovou dimenzí (počet detekcí za den).
    \item \textbf{Dotaz 3:} Spojení více tabulek s filtrováním a řazením (průměrná rychlost pro 10 měst v ČR pro vozidla třídy '2').
    \item \textbf{Dotaz 4:} Agregace nad více dimenzemi (průměrná rychlost podle typu detekce a třídy vozidla).
    \item \textbf{Dotaz 5:} Komplexní dotaz s CTE a okenní funkcí (průměrná rychlost o víkendech a rankování měst).
    \item \textbf{Dotaz 6:} Načtení všech dat z faktové tabulky (\texttt{Select * from FactCameraDetection}).
\end{itemize}

\subsubsection{Nástroje pro měření}

Měření časů probíhalo dvěma způsoby:
\begin{enumerate}
    \item \textbf{Přímé měření v databázi:} Doba provedení (Execution Time) každého z testovacích SQL dotazů byla měřena pomocí nativních nástrojů databázových klientů -- \texttt{SQL Server Management Studio} pro MS SQL a \texttt{DBeaver} pro ostatní databáze.
    \item \textbf{Měření v BI nástroji:} Měřila se celková doba načtení a vykreslení dashboardu. V \texttt{Power BI} byl využit nástroj "Performance Analyzer", zatímco v \texttt{Supersetu} byly pro měření použity vývojářské nástroje webového prohlížeče.
\end{enumerate}

\subsection{Fáze 1: Benchmark řádkového úložiště (Row-Store)}

V první fázi byly všechny databáze konfigurovány tak, aby využívaly své výchozí, řádkově orientované úložiště (např. \texttt{InnoDB} v případě MariaDB). Byly provedeny všechny testovací dotazy a měření v BI nástrojích. Získané časy byly zaznamenány a slouží jako základní výkonnostní baseline pro srovnání.

\subsection{Fáze 2: Benchmark sloupcového úložiště (Column-Store)}

Ve druhé fázi byla provedena konverze faktové tabulky \texttt{FactCameraDetection} z řádkového na sloupcové úložiště. Tento proces se lišil v závislosti na použité databázové platformě:
\begin{itemize}
    \item U \textbf{MS SQL Serveru} stačilo aplikovat clusterovaný columnstore index, což je relativně přímočará operace.
    \item U \textbf{MariaDB} byl proces složitější. Bylo nutné nejprve manuálně odstranit všechny existující B-stromové indexy a cizí klíče z faktové tabulky, a teprve poté bylo možné změnit úložný engine na \texttt{ColumnStore} pomocí příkazu \texttt{ALTER TABLE ... ENGINE=ColumnStore;}.
\end{itemize}

Po úspěšné konverzi na sloupcové úložiště byla celá sada testů (přímé dotazy i načítání dashboardů) provedena znovu a časy byly opět zaznamenány.

\subsection{Vyhodnocení výsledků}

Získaná data z obou fází měření umožňují provést detailní srovnání. Analýza se zaměří na vyhodnocení přínosu sloupcového úložiště pro každou platformu a na přímé srovnání výkonu mezi MS SQL Serverem a MariaDB v obou konfiguracích. Výsledky budou prezentovány a okomentovány v následující kapitole.
