...
Pro praktickou část byly vybrány technologie pokrývající komerční i~open-source přístupy. Tabulka níže shrnuje jejich charakteristiku a~roli v~navržené architektuře.
...
\textbf{MariaDB} \par
MariaDB je open-source relační databázový systém, který vznikl jako fork MySQL a~zachovává si s~ním vysokou kompatibilitu. Pro potřeby této práce je klíčové jeho rozšíření \emph{MariaDB ColumnStore}, které transformuje tradiční řádkově orientovanou databázi na hybridní systém schopný sloupcového ukládání dat. Tato architektura je optimalizována pro analytické dotazy (OLAP).
& 
% --- KOMERČNÍ DWH ---
...
SSAS je komponenta Microsoft SQL Serveru určená pro OLAP a~Business Intelligence. V~této práci je využíván jeho \emph{Tabulární model (Tabular Model)}, který funguje jako \emph{in-memory} databáze optimalizovaná pro analytické dotazy pomocí jazyka \texttt{DAX} (Data Analysis Expressions). Slouží jako komerční protějšek k~Cube.js, poskytující podobnou sémantickou vrstvu s~pokročilými možnostmi bezpečnosti, správy a~hlubokou integrací s~nástroji Microsoft ekosystému.
\\ 
\bottomrule
...
Detailní znění všech použitých SQL dotazů je k~dispozici v~příloze na githubu.
...
\begin{figure}[h]
  ...
  \caption{Porovnání velikosti dat faktové tabulky na disku mezi jednotlivými databázovými systémy. Zdroj: vlastní s~použitím programovacího jazyka Python}
...
\begin{figure}[H]
  ...
  \caption{Propustnost databází (TPS) v~závislosti na počtu souběžných uživatelů. Vyšší je lepší. Zdroj: vlastní s~použitím programovacího jazyka Python}
...
\begin{itemize}
    \item \textbf{ETL procesy:} Byla vytvořena sada optimalizovaných Python skriptů pro extrakci a~transformaci dat.
    \item \textbf{Datové sklady (DWH):} Práce poskytuje srovnání a~implementační schémata pro čtyři různé databázové enginy (MS SQL, MariaDB, PostgreSQL, ClickHouse), včetně optimalizačních technik (indexace, komprese).
    \item \textbf{Sémantická vrstva:} Byly navrženy a~ověřeny dva přístupy k~definici datových modelů – tradiční SSAS Tabular (Microsoft) a~moderní „Headless BI“ přístup pomocí Cube.js.
...
\begin{itemize}
    \item \textbf{Výkonnostní srovnání:} Testy prokázaly, že sloupcová databáze ClickHouse dokáže v~analytických dotazech (OLAP) nejen konkurovat, ale v~mnoha případech i~překonat in-memory technologii MS SQL Server Analysis Services (SSAS). To vyvrací mýtus, že pouze drahá in-memory řešení jsou vhodná pro rychlou interaktivní analýzu.
