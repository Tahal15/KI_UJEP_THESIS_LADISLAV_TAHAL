...
&
\cellcolor{comred}
    \textbf{Microsoft SQL Server Analysis Services (SSAS)} \par
...
\subsubsection{Automatizovaná analýza JSON struktur pomocí skriptu analyze\_json.py}
...
\begin{figure}[h]
  ...
  \caption{Tyto logické skupiny byly navíc ještě manuálně sloučeny napřič skupiny}
  \label{fig:topic_groups_merged}
\end{figure}

Na základě výsledků byly pro další zpracování vybrány skupiny témat označené žlutě a oranžově, které vykazovaly největší logickou podobnost a datový objem.
...
Klíčovým polem v těchto datech je \texttt{payload}, obsahující JSON strukturu s detaily detekce. Pro jeho zpracování byly v rámci práce implementovány dva odlišné přístupy:

\begin{itemize}
    \item \textbf{Statické ETL (SQL Server, MariaDB, MariaDB+ClickHouse):} Tento přístup byl implementován pro data z kamerového systému města Bílina (\texttt{/Bilina/kamery/camea/\%}). Vytvořené skripty využívají předem definovanou znalost struktury JSON dat. Atributy z \texttt{payload} jsou mapovány na pevně dané sloupce ve staging tabulce. Jedná se o rigidní, ale vysoce efektivní metodu, vhodnou pro konkrétní, neměnné datové zdroje, kde je prioritou maximální výkon a předvídatelnost zpracování.

    \item \textbf{Dynamické ETL (PostgreSQL s TimescaleDB):} Naopak pro implementaci nad databází PostgreSQL byl zvolen univerzální přístup. Skript (\texttt{pg\_timescale\_lake\_to\_staging.py}) byl navržen tak, aby se dokázal automaticky adaptovat na různorodé a vyvíjející se JSON struktury z různých MQTT témat. Během zpracování dynamicky analyzuje obsah \texttt{payload}, pomocí funkce \texttt{infer\_pg\_type} odvozuje datové typy jednotlivých atributů a v případě potřeby sám upravuje cílovou staging tabulku příkazem \texttt{ALTER TABLE}, aby přidal nové sloupce. Tento flexibilní mechanismus umožňuje zpracovávat data i z dosud neznámých senzorů bez nutnosti jakékoliv změny v kódu.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Krok 1: Z datového jezera do stagingu (\texttt{bilina\_kamery\_lake\_to\_staging.py})} \\
    Tento skript využívá knihovnu \texttt{pyodbc}. Inkrementálně načítá surová data z tabulky \texttt{mqttentries}, parsuje JSON \texttt{payload} a očištěné záznamy vkládá po dávkách do staging tabulky \texttt{[Stg].[CameraCamea]}. Každý běh je logován v tabulce \texttt{ETL\_RunLog}, přičemž pro získání ID běhu efektivně využívá T-SQL klauzuli \texttt{OUTPUT inserted.RunID}.

    \item \textbf{Krok 2: Ze stagingu do dimenzí a faktů (\texttt{bilina\_kamery\_staging\_to\_fact.py})} \\
    Druhý skript zpracovává data připravená ve stagingu. Pro efektivní nalezení nových dimenzních hodnot nejprve vloží unikátní záznamy z dávky do dočasné tabulky \texttt{\#\#TempDimensions}. Následně pomocí T-SQL klauzule \texttt{EXCEPT} porovná obsah dočasné tabulky s existujícími dimenzemi a vloží pouze chybějící záznamy. Poté konstruuje záznamy pro faktovou tabulku \texttt{FactCameraDetection} pomocí \texttt{LEFT JOIN} na dimenze pro získání číselných klíčů.
\end{itemize}

\subsection{MariaDB}

Implementace pro MariaDB se řídí stejnou dvoukrokovou logikou, avšak krok 2 je výrazně optimalizován pro specifika enginu ColumnStore.
...
\begin{itemize}
    ...
    Tento skript používá pro plnění dimenzí efektivní příkaz \texttt{INSERT IGNORE}, který přeskočí již existující záznamy. Klíčová optimalizace nastává při plnění faktové tabulky: skript nejprve v Pythonu načte všechny dimenzní klíče do paměti, následně projde staging data, nahradí původní hodnoty číselnými klíči a celý výsledek zapíše do dočasného CSV souboru. Nakonec je tento soubor nahrán do faktové tabulky pomocí příkazu \texttt{LOAD DATA LOCAL INFILE}, což je doporučená metoda pro hromadné vkládání do MariaDB ColumnStore.
    MariaDB v engine=columnstore nepodporuje auto inkrement. Tato logika je tedy řešena přímo v ETL scriptu.
\end{itemize}

\subsection{PostgreSQL (TimescaleDB)}

ETL proces pro PostgreSQL je unikátní svým dynamickým přístupem v prvním kroku a efektivním hromadným vkládáním ve druhém.

\begin{itemize}
    \item \textbf{Krok 1: Dynamické ETL z jezera do stagingu (\texttt{pg\_timescale\_lake\_to\_staging.py})} \\
    Tento skript automaticky analyzuje JSON \texttt{payload}. Pomocí funkcí \texttt{flatten\_json()} a \texttt{infer\_pg\_type()} dynamicky odvozuje schéma a datové typy. Pokud narazí na nový atribut, sám upraví cílovou staging tabulku příkazem \texttt{ALTER TABLE}. Tím je zajištěna schopnost zpracovat i data z nových senzorů bez úpravy kódu.

    \item \textbf{Krok 2: Ze stagingu do dimenzí a faktů (\texttt{pg\_timescale\_staging\_to\_fact.py})} \\
    Druhý skript využívá pro plnění dimenzí specifickou vlastnost PostgreSQL – příkaz \texttt{INSERT ... ON CONFLICT DO NOTHING}. Tento příkaz je spouštěn pomocí optimalizační funkce \texttt{execute\_values} z knihovny \texttt{psycopg2}. Její efektivita spočívá v tom, že namísto odesílání velkého množství jednotlivých \texttt{INSERT} příkazů (jeden pro každý záznam) sestaví jediný rozsáhlý \texttt{INSERT} příkaz, který obsahuje všechny hodnoty najednou. Tím se dramaticky snižuje komunikační režie mezi aplikací a databázovým serverem a umožňuje to databázi zpracovat celou dávku v jediné operaci. Tímto způsobem je zajištěno velmi rychlé vložení pouze unikátních hodnot. Následně jsou data ze stagingu pomocí standardního \texttt{INSERT INTO ... SELECT} s \texttt{LEFT JOIN}y transformována do faktové tabulky.
\end{itemize}

\subsection{MariaDB + ClickHouse}

Tato varianta je optimalizována na maximální výkon, přičemž většina logiky kroku 2 je přenesena přímo do SQL dotazů v ClickHouse.

\begin{itemize}
\item \textbf{Krok 1: Z datového jezera do stagingu (\texttt{maria\_click\_bilina\_kamery\_lake\_to\_staging.py})} \\
    Proces kombinuje připojení na MariaDB (datové jezero) a ClickHouse (staging). Pro čtení z MariaDB využívá server-side kurzor (\texttt{SSCursor}), který minimalizuje paměťové nároky. Data jsou do ClickHouse zapisována po dávkách pomocí nativního binárního protokolu, což výrazně zrychluje přenos. Logovací a kontrolní tabulky jsou vedeny přímo v ClickHouse.
