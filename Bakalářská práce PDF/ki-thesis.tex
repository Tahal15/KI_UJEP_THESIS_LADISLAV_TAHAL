...
\subsection{Microsoft Data Platform (komerční, on-premise/cloud)}
   ...
   \begin{figure}[h]
      ...
      \caption{Architektura moderního datového skladu v~prostředí Microsoft Azure. Zdroj: learn.microsoft.com \protect\cite{azure_data_warehousing}}
...
\begin{itemize}
    \item \textbf{Tradiční on-premise infrastruktura:} Základem je \emph{Microsoft SQL Server}. Pro integraci dat (ETL) je využíván modul \emph{SQL Server Integration Services} (SSIS) a pro analytické modelování (OLAP) slouží \emph{SQL Server Analysis Services} (SSAS). Toto složení tvoří osvědčený standard pro lokální datové sklady \cite{mssql2024overview}.
    ...
    \item \textbf{Vizuální a sémantická vrstva:} Sjednocující vrstvou pro on-premise i cloud je \emph{Power BI}. Tento nástroj slouží pro tvorbu reportů a dashboardů a díky své sémantické vrstvě dokáže zastoupit i některé funkce SSAS \cite{powerbi2024docs}.
...
\subsection{Google Cloud Data Analytics (komerční, cloud)}
   ...
   \begin{figure}[h]
      ...
      \caption{Architektura analytické platformy Google Cloud. Zdroj: medium.com \protect\cite{google_data_products}}
...
\begin{itemize}
    \item \textbf{Datový sklad:} Jádrem platformy je \emph{Google BigQuery}, serverless datový sklad s~kolumnárním uložením, kde se platí pouze za uložená data a zpracované dotazy \cite{bigquery2024docs}.
...
\subsection{Amazon Web Services (komerční, cloud)}
    ...
    \begin{figure}[h]
      ...
      \caption{Cloudové řešení Amazon Web Services. Zdroj: docs.aws.amazon.com \protect\cite{aws_redshift_arch}}
...
\begin{itemize}
    \item \textbf{Datový sklad:} Klíčovou roli zde zastává \emph{Amazon Redshift} -- cloudový datový sklad postavený na kolumnární architektuře \cite{redshift2024overview}.
...
\subsection{Oracle Data Platform (komerční, cloud/on-premise)}
   ...
   \begin{figure}[h]
      ...
      \caption{Architektura Oracle Data Platform. Zdroj: medium.com \protect\cite{oracle_data_platform}}
...
\subsection{Ekosystém Apache Software Foundation}
  ...
  \begin{figure}[h]
      ...
      \caption{Cloudové řešení Apache Software Foundation. Zdroj: blogs.perficient.com \protect\cite{hadoop_ecosystem_perficient}}
...
\subsection{Platforma MariaDB (Community / Enterprise)}
  ...
  \begin{figure}[h]
      ...
      \caption{Architektura řešení MariaDB. Zdroj: smartdatainstitute.com \protect\cite{mariadb_smartdatainstitute}}
...
\begin{figure}[h]
  ...
  \caption{Model context protocol. Zdroj: modelcontextprotocol.io \cite{mcp_protocol}}
  \label{fig:gemini_cli_architecture}
\end{figure}
...
\begin{itemize}
 ...
 \caption{Schéma hvězda faktové tabulky a dimenzí pro data z~kamer v~Bílině. Zdroj: en.wikipedia.org \cite{wikipedia_star_schema}}
\end{figure}
\clearpage
  ...
  \caption{Ilustrační schéma snéhové vločky faktové tabulky a dimenzí. Zdroj: en.wikipedia.org \cite{wikipedia_snowflake_schema}}
...
\begin{figure}[h]
  ...
  \caption{Struktura tabulky \texttt{mqttentries} obsahující zdrojová telemetrická data. Zdroj: Dbeaver}
...
\begin{figure}[h]
  ...
  \caption{Prvotní seskupení témat na základě 50\,\% strukturální podobnosti JSON schémat. Zdroj: Excel}
...
\begin{figure}[h]
  ...
  \caption{Tyto logické skupiny byly navíc ještě manuálně sloučeny napříč skupiny. Zdroj: Excel}
...
\begin{figure}[h]
  ...
  \caption{Staging tabulka v~datovém skladu. Zdroj: DBeaver}
  \label{fig:mariadwstaging}
\end{figure}

\clearpage

\section{Struktura a vrstvy datového skladu}
...
Každá implementace využívá dávkové zpracování (\texttt{BATCH\_SIZE}) a kontrolu posledního zpracovaného záznamu (\texttt{LastLoadedID}), aby bylo možné ETL proces spouštět opakovaně a inkrementálně.
 ...
 \caption{Schéma hvězda faktové tabulky a dimenzí pro data z~kamerového systému firmy CAMEA Technology, a. s. na datové platformě Portabo}
\end{figure}
...
\subsection{Komerční varianta -- SSAS Tabular a Power BI}
...
\begin{enumerate}
    ...
    \item \textbf{Dotaz 2: Agregace nad časovou dimenzí}
      ...
      \label{fig:srovnani2}
    \end{figure}
    Druhý dotaz (počet detekcí v~čase) odhalil zajímavé chování. Ačkoliv sloupcové databáze dominovaly, řádková MariaDB zde dosáhla překvapivě dobrého výsledku. To je způsobeno efektivním využitím B-stromového indexu na sloupci `TimeKey`, díky kterému engine nemusel provádět sken celé tabulky.
    ...
    Třetí, komplexnější dotaz, demonstroval sílu optimalizátorů MS SQL a ClickHouse, které efektivně aplikují filtry (predicate pushdown) ještě před samotným spojením tabulek.
...
\begin{figure}[h!]
  ...
  \caption{Celkové porovnání výkonu na úrovni databáze. Méně je lépe}
  \label{fig:srovnani6}
\end{figure}
...
\subsection{Srovnání výkonu a architektury BI nástrojů}
...
\subsection{Závěrečné shrnutí výsledků}
...
\begin{figure}[h!]
  ...
  \caption{Power BI Performance Analyzer - analýza vykreslování}
  \label{fig:srovnani5}
\end{figure}
...
Závěrem lze říci, že open-source stack nabízí obrovskou flexibilitu a škálovatelnost, je však vykoupena vyššími nároky na technickou expertizu, zatímco komerční platformy nabízejí okamžitý výkon in-memory modelu výměnou za licenční poplatky a hardwarová omezení.
\subsection{Testování souběžné zátěže (Concurrency)}

Kromě měření rychlosti odezvy pro jednoho uživatele bylo provedeno i teoretické zhodnocení chování systémů při souběžném přístupu více uživatelů (concurrency). Tento aspekt je kritický pro reálné nasazení, kde s~dashboardy pracuje více analytiků současně.

\begin{itemize}
    \item \textbf{MS SQL Server + SSAS (MOLAP):} Díky in-memory architektuře zvládá SSAS vysokou míru souběžnosti velmi efektivně. Data jsou načtena v~paměti a dotazy jsou obsluhovány přímo z~ní, což eliminuje zámky na disku. Limitujícím faktorem je zde pouze propustnost CPU a paměti. Pro běžný provoz (desítky uživatelů) je výkon prakticky neovlivněn.
    \item \textbf{ClickHouse + Cube.js (ROLAP):} ClickHouse je navržen pro extrémně rychlé čtení, ale jeho architektura (LSM stromy) není primárně optimalizována pro tisíce malých souběžných dotazů (high QPS). Při vysoké zátěži může docházet k~frontám požadavků. Zde hraje klíčovou roli \textbf{Cube.js}, která funguje jako cache a load balancer. Díky pre-agregacím a cachování výsledků v~Redis vrstvě se většina opakujících se dotazů vůbec nedostane k~databázi, čímž se systém stává vysoce odolným i proti náhlým špičkám v~návštěvnosti.
\end{itemize}

\subsection{Práce s~mapovými daty}

Součástí zadání byla i analýza práce s~geoprostorovými daty, která jsou v~kontextu Smart City (kamery, senzory) klíčová. Oba testované nástroje nabízejí možnosti vizualizace na mapě, avšak s~rozdílným přístupem.

\begin{itemize}
    \item \textbf{Power BI (Azure Maps / Bing Maps):}
    Nabízí hlubokou integraci s~mapovými podklady Microsoftu. Umožňuje snadné vykreslení bodů (kamery) i oblastí (městské části). Výhodou je intuitivní ovládání a automatické geokódování. Nevýhodou může být limit na počet vykreslených bodů v~základní verzi a nutnost placené licence pro pokročilé GIS funkce.
    
    \item \textbf{Apache Superset (Deck.gl):}
    Využívá knihovnu \textit{Deck.gl} od Uberu, která je standardem pro vizualizaci velkých objemů geodat. Superset dokáže plynule vykreslit tisíce bodů (např. trajektorie vozidel) nebo heatmapy hustoty dopravy. Oproti Power BI nabízí větší flexibilitu v~nastavení vrstev (layers) a typů vizualizací (Arc, Polygon, Scatterplot), vyžaduje však, aby data v~databázi obsahovala správné souřadnice (latitude/longitude) a jejich napojení je technicky náročnější.
\end{itemize}

\subsection{Srovnání vizualizačních možností}

Zatímco výkon je objektivně měřitelná veličina, kvalita a šíře vizualizačních možností ovlivňuje především uživatelskou přívětivost a schopnost efektivně předat informaci.

\begin{itemize}
    \item \textbf{Microsoft Power BI:}
    Představuje špičku v~oblasti "self-service BI". Nabízí obrovské množství nativních vizuálů a ještě širší knihovnu komunitních doplňků (AppSource).
    \begin{itemize}
        \item \textbf{Interaktivita:} Vyniká v~tzv. \textit{cross-filtering}, kdy kliknutí do jednoho grafu automaticky filtruje ostatní. Funkce \textit{Drill-through} umožňuje snadný přechod z~agregovaného pohledu na detailní data.
        \item \textbf{Design:} Umožňuje pixel-perfect design reportů, pokročilé formátování a tvorbu mobilních layoutů.
    \end{itemize}

    \item \textbf{Apache Superset:}
    Zaměřuje se spíše na rychlou exploraci dat než na dokonalý design reportů.
    \begin{itemize}
        \item \textbf{Vizualizace:} Nabízí bohatou sadu grafů (ECharts), včetně specializovaných (Sankey diagramy, Sunburst, Word Cloud).
        \item \textbf{SQL Lab:} Unikátní funkcí je integrované SQL IDE, které umožňuje analytikům psát přímé dotazy a okamžitě vizualizovat výsledky, což v~Power BI chybí (tam je nutné jít přes Power Query nebo DAX).
        \item \textbf{Limitace:} Možnosti formátování (barvy, fonty, rozložení) jsou oproti Power BI omezené. Interaktivita mezi grafy (cross-filtering) je podporována, ale její nastavení není tak intuitivní.
    \end{itemize}
\end{itemize}

\subsection{Analýza celkových nákladů na vlastnictví (TCO)}
...
TCO v~tomto případě dělíme na tři hlavní složky: \textbf{Licenční náklady}, \textbf{Infrastrukturní náklady} a \textbf{Náklady na lidský kapitál}.

\subsubsection{Náklady na implementaci a lidský kapitál}

\begin{itemize}
    \item \textbf{Srovnatelná pracnost implementace ETL:} Jelikož byla pro oba přístupy (komerční i open-source) zvolena strategie ETL s~využitím jazyka \textbf{Python}, objem práce nutný pro napsání ETL procesů byl v~podstatě totožný. Psaní skriptu pro \texttt{pyodbc} (MSSQL) vyžadovalo srovnatelnou časovou investici jako psaní skriptu pro \texttt{clickhouse-driver}.
    \item \textbf{Rozdíl v~expertíze:} Hlavní rozdíl tedy nespočívá v~objemu práce, ale v~typu požadované znalosti. Zatímco správa MSSQL je na trhu běžnou komoditou, efektivní konfigurace \textbf{MariaDB Columnstore} technologie nebo ladění \textbf{Cube.js} schémat vyžaduje specifičtější, a často dražší, know-how.
\end{itemize}

Pro kvantifikaci těchto nákladů byly využity průměrné mzdy v~IT sektoru dle dat Českého statistického úřadu (ČSÚ) a Informačního systému o~průměrném výdělku (ISPV).

\begin{table}[h!]
\centering
\caption{Odhad měsíčních nákladů na lidský kapitál dle rolí}
\label{tab:personnel_costs}
\begin{tabular}{|l|l|c|}
\hline
\textbf{Technologie} & \textbf{Role} & \textbf{Odhad mzdy (CZK)} \\ \hline
MariaDB ColumnStore & Senior Database Specialist & 90 000 \\ \hline
Superset (Vizualizace) & Junior Data Analyst & 45 000 \\ \hline
MS SQL + SSAS & BI Developer / DWH Specialist & 70 000 \\ \hline
\end{tabular}
\end{table}

Z~tabulky vyplývá, že ačkoliv open-source řešení šetří na licencích, může vyžadovat dražší specialisty na backend (Senior DB Specialist pro správu ColumnStore clusteru), zatímco pro frontend (Superset) postačí juniorní role. Komerční ekosystém MS SQL obvykle pokrývá jedna role BI Developera se střední úrovní ohodnocení.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{SQL Server Standard:} Cena se pohybuje okolo \textbf{3 500 -- 4 000 USD} za balík 2 jader. Tato edice je však limitována maximálně \textbf{128 GB RAM} pro databázový engine a \textbf{64 GB RAM} pro SSAS (Tabular).
        \item \textbf{SQL Server Enterprise:} Pro využití větší paměti (nutné pro velké in-memory modely) je nutná Enterprise edice, kde cena strmě stoupá (cca \textbf{15 000 USD} za 2 jádra).
    \end{itemize}

    \item \textbf{Infrastrukturní dopad (In-Memory daň):}
    Jelikož SSAS Tabular drží data v~paměti, naráží u~velkých objemů dat (Big Data) na hardwarové a licenční limity. Pokud datový model přesáhne 64 GB (limit Standard edice), organizace je nucena přejít na násobně dražší Enterprise edici, což skokově zvyšuje TCO.
...
\begin{itemize}
    \item \textbf{MOLAP (MS SSAS):} Vyžaduje, aby se celý datový model (nebo jeho podstatná část) vešel do operační paměti (RAM). RAM je nejdražší komponentou serveru. S~růstem dat rostou náklady na RAM lineárně až exponenciálně.
...
\subsubsection{Závěrečné porovnání}
...
\begin{table}[h!]
\centering
\caption{Souhrnné porovnání TCO a parametrů řešení}
\label{tab:tco_summary}
\begin{tabular}{|l|p{4cm}|p{4cm}|p{3cm}|}
\hline
\textbf{Parametr} & \textbf{MS SQL + SSAS (Komerční)} & \textbf{ClickHouse + Cube.js (Open-source)} & \textbf{MariaDB (Tradiční)} \\ \hline
\textbf{Licence} & Vysoká (per core / user) & Zdarma (Apache 2.0 / MIT) & Zdarma (GPL) \\ \hline
\textbf{Hardware} & Vysoké nároky na RAM (MOLAP) & Nízké nároky na RAM, vyšší I/O (ROLAP) & Střední \\ \hline
\textbf{Storage} & Standardní komprese & Extrémní komprese (10x menší) & Bez komprese (Row) \\ \hline
\textbf{Lidské zdroje} & BI Developer (Střední mzda) & Senior DB Spec. (Vysoká) + Junior (Nízká) & DB Admin (Střední) \\ \hline
\textbf{Výkon} & Okamžitý (In-Memory) & Velmi vysoký (Columnar) & Nízký (pro OLAP) \\ \hline
\textbf{Škálovatelnost} & Omezená licencí a RAM & Neomezená (Horizontální) & Omezená \\ \hline
\end{tabular}
\end{table}
...
Tato bakalářská práce se zabývala problematikou využití open-source a komerčních nástrojů pro vizualizaci a analýzu dat v~kontextu datové platformy Portabo. Hlavním cílem bylo provést srovnávací analýzu vybraných technologií z~hlediska jejich výkonu, funkčních vlastností, uživatelské přívětivosti a ekonomické efektivity.

V~teoretické části byly vymezeny základní pojmy z~oblasti datových skladů, OLAP technologií a Business Intelligence. Byla provedena rešerše dostupných řešení, která posloužila jako základ pro výběr testovaných technologií.

V~praktické části byla navržena a implementována modulární datová architektura, která umožnila přímé srovnání několika přístupů. Byla vytvořena řešení postavená na databázových systémech Microsoft SQL Server, MariaDB (včetně ColumnStore enginu), PostgreSQL s~rozšířením TimescaleDB a ClickHouse. Nad těmito datovými sklady byly implementovány analytické vrstvy využívající jak komerční platformu Microsoft SSAS a Power BI, tak open-source stack tvořený nástroji Cube.js a Apache Superset.

Provedená srovnávací analýza přinesla následující klíčová zjištění:

\begin{itemize}
    \item \textbf{Výkon:} Komerční řešení postavené na in-memory technologii (MS SSAS) prokázalo bezkonkurenční rychlost odezvy ("out-of-the-box"), avšak za cenu vysokých nároků na operační paměť. Open-source varianta využívající ClickHouse a Cube.js se tomuto výkonu dokázala přiblížit díky využití sloupcového ukládání a pre-agregací, přičemž nabídla výrazně lepší škálovatelnost při práci s~velkými objemy dat.
    \item \textbf{Ekonomické aspekty:} Analýza TCO ukázala, že zatímco komerční řešení je výhodné pro menší projekty díky nižší pracnosti implementace a integrovanému prostředí, s~růstem objemu dat a počtu uživatelů se stává ekonomicky neudržitelným kvůli licenčním poplatkům. Open-source řešení naopak vyžaduje vyšší počáteční investici do lidského kapitálu (know-how), ale jeho provozní náklady jsou při škálování výrazně nižší.
    \item \textbf{Flexibilita:} Open-source stack prokázal vysokou míru flexibility a modularity, což umožňuje snadnou výměnu komponent a přizpůsobení specifickým potřebám platformy Portabo.
\end{itemize}

Na základě dosažených výsledků lze pro datovou platformu Portabo doporučit hybridní přístup. Pro kritické reporty vyžadující maximální rychlost a integraci s~kancelářskými aplikacemi je vhodné využít komerční nástroje Microsoft. Pro zpracování velkých objemů telemetrických dat a veřejné dashboardy se však jako optimální jeví využití výkonných open-source technologií v~čele s~databází ClickHouse a sémantickou vrstvou Cube.js, které nabízejí nejlepší poměr cena/výkon a neomezenou škálovatelnost.

Práce tak splnila své cíle a poskytla ucelený pohled na možnosti moderních datových platforem, který může sloužit jako vodítko pro budoucí rozvoj datové infrastruktury v~regionu.
