...
V první fázi byl měřen čas provedení sady pěti reprezentativních analytických dotazů. Cílem bylo porovnat efektivitu tradičního řádkového (row-store) a moderního sloupcového (column-store) úložiště.
...
Graf \ref{fig:srovnani1} ukazuje výsledky pro základní analytický dotaz (průměrná rychlost dle třídy vozidla). Zde se naplno projevuje klíčová výhoda sloupcových databází (ClickHouse, MSSQL Col., MariaDB Col.), které načítají pouze relevantní sloupce. Naopak tradiční řádkové úložiště (MariaDB Row) ve výchozím nastavení načítá celé řádky, což při milionech záznamů vede k masivnímu I/O a pomalé odezvě.
...
\begin{figure}[h!]
  \centering
   Zde vložte graf porovnávající "MariaDB Před" a "MariaDB Po" optimalizaci
...
Jak ukazuje graf \ref{fig:mariadb_opt}, po aplikaci těchto specifických indexů došlo k řádovému zrychlení:
\begin{itemize}
