...
Na rozdíl od předchozích výkonnostních testů, které srovnávaly různé varianty úložných enginů (řádkové vs. sloupcové), byly pro tento zátěžový test vybrány pouze \textbf{nejefektivnější konfigurace} každé databázové platformy. Cílem bylo porovnat maximální potenciál jednotlivých technologií v~jejich optimálním nastavení pro analytickou zátěž (např. MariaDB s~enginem ColumnStore, MSSQL s~Columnstore indexy).
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{SQL Server Standard:} Tato edice je cenově dostupnější (cca 3\,500--4\,000 USD za 2 jádra). Maximální využitelná paměť pro analytickou službu \textbf{SSAS (Tabular) je limitována na 64 GB RAM} na instanci (od verze 2022) \cite{ms_sql_editions}. To je sice hardwarově dostačující pro střední modely, ale tato edice neumožňuje provozovat on-premise Power BI Report Server.
        \item \textbf{SQL Server Enterprise:} Pro modely překračující 64 GB RAM nebo pro nárok na \textbf{Power BI Report Server} je nutná edice Enterprise. Cena této edice je přibližně čtyřnásobná oproti verzi Standard (cca 14\,000 USD a více za 2 jádra).
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Power BI Pro (Cloud):} Pro hybridní scénář je nutná licence cca \textbf{10 USD / uživatel / měsíc}. Tato licence je součástí pouze nejvyšších plánů Office 365 (E5), u~běžných plánů (E3, Business Standard) je nutné ji dokoupit.
