...
\begin{figure}[h!]
  \centering
  \includegraphics[width=\textwidth]{img/agregace-maria.png} 
  \caption{Dopad nasazení pokrývajících indexů na výkon MariaDB (InnoDB)}
  \label{fig:mariadb_opt}
\end{figure}

Jak ukazuje obrázek \ref{fig:mariadb_opt} ukazuje, jaký dopad měla optimalizace pokrývajících indexů na výkon MariaDB (InnoDB). Po aplikaci těchto specifických indexů došlo k řádovému zrychlení:
\begin{itemize}
    \item U komplexní analýzy víkendů (Dotaz 5) klesl čas z \textbf{2 minut 22 sekund} na pouhé \textbf{3 sekundy}.
    \item U agregace dle typu detekce (Dotaz 4) došlo ke zrychlení z \textbf{1 minuty 45 sekund} na \textbf{7 sekund}.
\end{itemize}
Tento experiment potvrzuje, že MariaDB je schopna sloužit i pro analytické dotazy, vyžaduje však – na rozdíl od sloupcových databází nebo PostgreSQL – striktní a manuální optimalizaci indexů na míru konkrétním dotazům.

\subsubsection{Interpretace výsledků databázové vrstvy}
Souhrnná analýza demonstruje několik klíčových zjištění:
\begin{itemize}
    \item \textbf{Dominance sloupcového úložiště:} Pro OLAP zátěž je sloupcový přístup (ClickHouse, MariaDB ColumnStore) bezkonkurenční, poskytující vysoký výkon bez nutnosti složitého ladění každého dotazu.
    ...
    \item \textbf{Výkon nativních OLAP databází:} \textbf{ClickHouse} dosahoval ve většině testů nejlepších výsledků s časy pod jednu sekundu.
    \item \textbf{Komerční řešení:} \textbf{MS SQL Server} s Columnstore indexem se výkonnostně velmi blížil ClickHouse a v některých případech jej překonal.
\end{itemize}
...
Bez nich byla odezva závislá čistě na výkonu databáze (u ClickHouse cca \textbf{5 sekund}). Implementace pre-agregací – tedy malých, předpočítaných sumarizačních tabulek – umožnila systému obcházet náročné výpočty nad hlavní databází. Díky tomu se podařilo srazit dobu odezvy dashboardu na přibližně \textbf{2 sekundy}, což představuje hranici akceptovatelnou pro interaktivní práci.
...
Tradiční předpoklad, že open-source řešení vyžadují výrazně vyšší úsilí při integraci se v kontextu této práce \textbf{nepotvrdil}.
