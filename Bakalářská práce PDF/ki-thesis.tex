...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textit{Permisivní licence}\footnote{Vysvětlení permisivních licencí: \url{https://opensource.org/licenses}} (např. MIT, Apache 2.0) jsou maximálně benevolentní, kladou minimální omezení a~obvykle umožňují upravený kód uzavřít a~následně komerčně prodávat jako proprietární produkt.
...
\begin{enumerate}
    ...
    Inmonova metodika je označována jako \textbf{Top-Down (shora dolů)}, protože začíná návrhem centrálního, podnikového datového skladu (\textit{Enterprise Data Warehouse}, EDW) \cite{inmon2005}.
    \begin{itemize}
...
\textit{Poznámka: Varianta MariaDB s~enginem InnoDB byla z~tohoto srovnání vyřazena, jelikož časy zpracování byly řádově vyšší a~provedení kompletního benchmarku by bylo časově neúměrné.}
...
Komerční řešení (Power BI) vyniká možností tvorby \textit{pixel-perfect} layoutů grafů a~hlubokou integrací s~kancelářskými nástroji, což ocení zejména běžní uživatelé a~management.
...
\begin{itemize}
    ...
    Pro nasazení ve firemním prostředí se obvykle volí model \textit{Per Core}, který je nezbytný pro externí přístup.
    \begin{itemize}
