...
\section{Využití AI pro validaci a revizi textu}

Nad rámec samotné implementace a srovnání datových platforem byly při tvorbě této práce aplikovány moderní nástroje umělé inteligence, konkrétně \textbf{Gemini CLI}, s~cílem zajistit maximální kvalitu a konzistenci výstupů. Tato sekce popisuje metodiku využití velkého jazykového modelu (LLM) pro automatizovanou revizi a demonstruje, jak lze prostřednictvím dodání relevantního kontextu ověřovat soulad mezi teoretickým textem práce a praktickými artefakty (zdrojové kódy, databázová schémata).

Standardní jazykové modely bez přístupu k~externím informacím nedokáží validovat specifické detaily konkrétního projektu, jelikož jim chybí znalost privátních dat. Pro efektivní analýzu je nezbytné modelu poskytnout tzv. \textbf{kontext} (uzemnění). Pro účely této práce bylo zvoleno řešení postavené na \textbf{Gemini CLI}, které umožnilo propojení modelu s~následujícími zdroji:

\begin{itemize}
    \item \textbf{Repozitář zdrojových kódů:} Kompletní projektová dokumentace a skripty uložené na službách \textbf{Google Drive} a \textbf{GitHub}.
    \item \textbf{Živá databázová spojení:} Přímé napojení na testovací instance databází (ClickHouse, MS SQL, PostgreSQL, MariaDB), což umožnilo modelu analyzovat aktuální strukturu (DDL) a metadata.
    \item \textbf{Text práce:} Zdrojový \LaTeX{} soubor, který sloužil jako referenční bod pro kontrolu obsahu.
\end{itemize}

Díky zpřístupnění tohoto kontextu bylo možné formulovat komplexní validační dotazy, například: \emph{„Je v~celém textu konzistentně dodržováno pojmenování ETL skriptů oproti repozitáři?“} nebo \emph{„Odpovídá popis architektury v~kapitole 3 reálné konfiguraci v~Docker souborech?“}.

Ačkoliv model Gemini disponuje rozsáhlým kontextovým oknem (přesahujícím 1 milion tokenů), pro efektivní analýzu je nutná optimalizace dotazů. Namísto načítání kompletního obsahu databáze (což by bylo výpočetně neefektivní) je vhodnější modelu zpřístupnit pouze schémata tabulek, indexy a systémové pohledy. Tímto způsobem lze nechat umělou inteligenci autonomně ověřit soulad mezi technickou implementací a jejím popisem v~textové části práce.


\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{img/Gemini-cli-architecture.png}
  \caption{Architektura propojení LLM s~lokálními zdroji (Component Diagram)}
  \label{fig:gemini_cli_uml}
\end{figure}
...
\chapter{SQL Dotazy k~srovnávací analýze}
\label{app:sql_queries}

Tato příloha obsahuje plné znění SQL dotazů použitých pro srovnávací analýzu výkonu databázových systémů.
