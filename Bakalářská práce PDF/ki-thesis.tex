...
Pro přehlednost jsou klíčové rozdíly mezi dvěma finálními kandidáty shrnuty v~následující tabulce. Databáze MariaDB a PostgreSQL byly z~finálního srovnání vyloučeny, neboť její výkonnostní limity pro účely OLAP analýzy nad velkými daty diskvalifikují.
...
Pro prostředí platformy Portabo, která pracuje s~objemnými telemetrickými daty z~IoT senzorů a kamerových či jiných systémů, se jako \textbf{vhodnější jeví open-source varianta postavená na kombinaci ClickHouse a Cube.js}.

Toto doporučení vychází z~následujících klíčových zjištění praktické části:

\begin{enumerate}
    \item \textbf{Efektivita práce s~daty:} ClickHouse prokázal v~testech schopnost ukládat data s~přibližně desetinásobnou úsporou diskového prostoru oproti tradičním systémům. Pro telemetrická data, která neustále narůstají, je tato vlastnost kritická.
    \item \textbf{Ekonomika škálování:} Licenční model Microsoftu (per-core) by se při nárůstu objemu dat a potřebě výpočetního výkonu stal pro tento typ projektu finančně neudržitelným. Open-source řešení umožňuje horizontální škálování na komoditním hardwaru bez licenčních poplatků.
    \item \textbf{Snadnost nasazení:} Praktická implementace ukázala významný rozdíl v~náročnosti instalace mezi open-source systémy. Zatímco ClickHouse fungoval v~režimu „out-of-the-box“ okamžitě po instalaci, zprovoznění MariaDB ColumnStore vyžadovalo komplexní konfiguraci. Tato provozní jednoduchost ClickHouse snižuje bariéru vstupu a nároky na správu.
    \item \textbf{Flexibilita integrace:} Nasazení Cube.js jako sémantické vrstvy umožňuje oddělit datovou logiku od vizualizace. To dává platformě Portabo svobodu v~budoucnu změnit vizualizační nástroj (např. nahradit Superset vlastní React aplikací) bez nutnosti přepisovat datové modely.
\end{enumerate}
...
|-- Bakalářská práce PDF/ .................. Text práce v systému LaTeX
|   `-- (zdrojové kódy LaTeX)
