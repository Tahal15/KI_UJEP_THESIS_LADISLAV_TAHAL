...
\begin{itemize}
    \item \textbf{Komerční software} je software, jehož distribuce a používání je podmíněno zakoupením licence. Jeho \textbf{zdrojový kód je uzavřený (proprietární)}, což znamená, že uživatelé nemají kód k dispozici, nemohou jej prohlížet, modifikovat, ani provádět zpětné inženýrství. Důsledkem je \textbf{nemožnost modifikovat} chování systému a potenciální \textbf{závislost na dodavateli} v otázkách úprav a oprav. Komerční řešení bývají vyvíjena a distribuována za účelem zisku. Výhodou bývá profesionální podpora, garantovaná kvalita a ucelený ekosystém nástrojů.

    \item \textbf{Open-source software} poskytuje uživatelům přístup k veřejně dostupnému zdrojovému kódu a uděluje jim práva k jeho používání, studování, modifikaci a šíření. Klíčovým prvkem je zde \textbf{open-source licence}, která přesně definuje, jakým způsobem lze s dílem a jeho modifikacemi nakládat. \textbf{Detailní pochopení licenčních podmínek je pro praktické nasazení zásadní} (pro přehled a podmínky licencí je nutné nahlédnout do oficiální dokumentace konkrétní licence).
    \begin{itemize}
        \item \textit{Permisivní licence} (např. \textbf{MIT}, \textbf{Apache 2.0}) jsou maximálně benevolentní, kladou minimální omezení a obvykle umožňují upravený kód uzavřít a následně komerčně prodávat jako proprietární produkt.
        \item \textit{Copyleftové licence} (např. \textbf{GPL}) jsou přísnější a vyžadují, aby jakékoliv odvozené dílo nebo úprava byly šířeny pod stejnou svobodnou licencí. Tím je zajištěno, že zdrojový kód zůstane trvale veřejně přístupný i v budoucích verzích.
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{figure}[h]
      ...
      \caption{Příklad možné plně open-source sestavy}
...
\begin{figure}[h]
  ...
  \caption{Návrh NLQ architektury}
...
\begin{figure}[h]
  ...
  \caption{Model context protocol - převzato z}
  \label{fig:gemini_cli_architecture}
\end{figure}
